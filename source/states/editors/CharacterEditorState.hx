...
class CharacterEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent {		
	...
	var animateGhostImage:String;
        var swfGhost:Dynamic = null; // placeholder for SWF ghost sprite
	#if openfl_swf
	var swf:SWFLite = cast(Assets.getLibrary("mylib"), SWFLite);
	...
	function addGhostUI() {
		...
		var makeGhostButton:PsychUIButton = new PsychUIButton(25, 15, "Make Ghost", function() {
			...
			if (!character.isAnimationNull()) {
				...
				} else if (myAnim != null && character.imageFile.endsWith(".swf")) {
					// --- SWF Ghost branch ---
					if (swfGhost == null) {
						var swfPath = Paths.getPath('images/' + character.imageFile, BINARY);
						#if sys
						if (sys.FileSystem.exists(swfPath)) {
							var bytes = sys.io.File.getBytes(swfPath);
							#if openfl_swf
							...
							#end
							#end
							insert(members.indexOf(ghost), swfGhost);
							swfGhost.alpha = ghostAlpha;
						}
						#end
					}
					if (swfGhost != null) {
						swfGhost.x = character.x;
						swfGhost.y = character.y;
						swfGhost.scaleX = character.scale.x;
						swfGhost.scaleY = character.scale.y;
						swfGhost.visible = true;
					}
				} else
					if (myAnim != null) // This is VERY unoptimized and bad, I hope to find a better replacement that loads only a specific frame as bitmap in the future.
				{
					...
					if (animateGhost == null || animateGhostImage != character.imageFile)
						Paths.loadAnimateAtlas(cast cast cast cast cast animateGhost, character.imageFile);
				...
				if (swfGhost != null)
					spr = swfGhost; // prefer SWF ghost if loaded
				if (spr != null) {
	...
	function reloadCharacterImage() {
		...
		if (Paths.fileExists('images/' + character.imageFile + '/Animation.json', TEXT)) {
			...
			try {
				Paths.loadAnimateAtlas(cast cast cast cast cast character.atlas, character.imageFile);
			} catch (e:Dynamic) {
